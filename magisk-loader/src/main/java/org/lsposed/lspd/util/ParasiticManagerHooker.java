...
import de.robv.android.msposed.XC_MethodHook;
import de.robv.android.msposed.XC_MethodReplacement;
...
public class ParasiticManagerHooker {
    ...
    private static void sendBinderToManager(final ClassLoader classLoader, IBinder binder) {
        try {
            var clazz = msposedHelpers.findClass("org.lsposed.manager.Constants", classLoader);
            var ok = (boolean) msposedHelpers.callStaticMethod(clazz, "setBinder",
    ...
    private static void hookForManager(ILSPManagerService managerService) {
        var managerApkHooker = new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                ...
                ApplicationInfo appInfo = (ApplicationInfo) msposedHelpers.getObjectField(bindData, "appInfo");
                msposedHelpers.setObjectField(bindData, "appInfo", getManagerPkgInfo(appInfo).applicationInfo);
            }
        };
        msposedHelpers.findAndHookMethod(ActivityThread.class,
                "handleBindApplication",
                "android.app.ActivityThread$AppBindData",
                managerApkHooker);

        var unhooks = new XC_MethodHook.Unhook[]{null};
        unhooks[0] = msposedHelpers.findAndHookMethod(
                LoadedApk.class, "getClassLoader", new XC_MethodHook() {
                    @Override
                    protected void afterHookedMethod(MethodHookParam param) {
                        var pkgInfo = getManagerPkgInfo(null);
                        if (pkgInfo != null && msposedHelpers.getObjectField(param.thisObject, "mApplicationInfo") == pkgInfo.applicationInfo) {
        ...
        var activityClientRecordClass = msposedHelpers.findClass("android.app.ActivityThread$ActivityClientRecord", ActivityThread.class.getClassLoader());
            ...
            protected void afterHookedMethod(MethodHookParam param) {
                for (var i = 0; i < param.args.length && activityClientRecordClass.isInstance(param.thisObject); ++i) {
                    if (param.args[i] instanceof ActivityInfo) {
                        ...
                        Hookers.logD("loading state of " + aInfo.name);
                        states.computeIfPresent(aInfo.name, (k, v) -> {
                            msposedHelpers.setObjectField(param.thisObject, "state", v);
                            return v;
                        });
                        persistentStates.computeIfPresent(aInfo.name, (k, v) -> {
                            msposedHelpers.setObjectField(param.thisObject, "persistentState", v);
                            return v;
                        });
                    }
                }
        ...
        msposedBridge.hookAllConstructors(activityClientRecordClass, activityHooker);

        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.O_MR1) {
            msposedBridge.hookAllMethods(msposedHelpers.findClass("android.app.ActivityThread$ApplicationThread", ActivityThread.class.getClassLoader()), "scheduleLaunchActivity", activityHooker);
        }

        msposedBridge.hookAllMethods(ActivityThread.class, "handleReceiver", new XC_MethodReplacement() {
            @Override
        ...
        msposedBridge.hookAllMethods(ActivityThread.class, "installProvider", new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                ...
                if (ctx != null && info != null && pkgInfo != null) {
                    ...
                    if (originalContext == null) {
                        ...
                        msposedHelpers.setObjectField(originalPkgInfo, "mPackageName", packageName);
                        originalContext = (Context) msposedHelpers.callStaticMethod(msposedHelpers.findClass("android.app.ContextImpl", null),
        ...
        msposedHelpers.findAndHookMethod(ActivityThread.class, "deliverNewIntents", activityClientRecordClass, List.class, new XC_MethodHook() {
            @Override
            protected void beforeHookedMethod(MethodHookParam param) {
        ...
        msposedHelpers.findAndHookMethod(WebViewFactory.class, "getProvider", new XC_MethodReplacement() {
            @Override
            protected Object replaceHookedMethod(MethodHookParam param) {
                var sProviderInstance = msposedHelpers.getStaticObjectField(WebViewFactory.class, "sProviderInstance");
                if (sProviderInstance != null) return sProviderInstance;
                //noinspection unchecked
                var providerClass = (Class<WebViewFactoryProvider>) msposedHelpers.callStaticMethod(WebViewFactory.class, "getProviderClass");
                ...
                try {
                    ...
                    msposedHelpers.setStaticObjectField(WebViewFactory.class, "sProviderInstance", sProviderInstance);
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                try {
                    ...
                    if (record instanceof IBinder) {
                        record = ((ArrayMap<?, ?>) msposedHelpers.getObjectField(param.thisObject, "mActivities")).get(record);
                        if (record == null) return;
                    }
                    msposedHelpers.callMethod(param.thisObject, Build.VERSION.SDK_INT >= Build.VERSION_CODES.P ? "callActivityOnSaveInstanceState" : "callCallActivityOnSaveInstanceState", record);
                    var state = (Bundle) msposedHelpers.getObjectField(record, "state");
                    var persistentState = (PersistableBundle) msposedHelpers.getObjectField(record, "persistentState");
                    var aInfo = (ActivityInfo) msposedHelpers.getObjectField(record, "activityInfo");
        ...
        msposedBridge.hookAllMethods(ActivityThread.class, "performStopActivityInner", stateHooker);
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.O_MR1)
            msposedHelpers.findAndHookMethod(ActivityThread.class, "performDestroyActivity", IBinder.class, boolean.class, int.class, boolean.class, stateHooker);
    }
