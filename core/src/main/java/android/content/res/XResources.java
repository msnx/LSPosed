...
import static de.robv.android.msposed.msposedHelpers.decrementMethodDepth;
import static de.robv.android.msposed.msposedHelpers.findAndHookMethod;
import static de.robv.android.msposed.msposedHelpers.getBooleanField;
import static de.robv.android.msposed.msposedHelpers.getLongField;
import static de.robv.android.msposed.msposedHelpers.getObjectField;
import static de.robv.android.msposed.msposedHelpers.incrementMethodDepth;
...
import de.robv.android.msposed.msposedHookZygoteInit;
import de.robv.android.msposed.XC_MethodHook;
import de.robv.android.msposed.XC_MethodHook.MethodHookParam;
 ...
 *
 * <p>msposed replaces the standard resources with this class, which overrides the methods used for
...
//			Log.w(msposedBridge.TAG, "Package name for " + resDir + " had to be retrieved via parser");
	 ...
	 * {@link msposedHookZygoteInit#initZygote initZygote()} for all apps. As there is no
	...
	private static void setReplacement(int id, Object replacement, XResources res) {
		...
		if (replacement instanceof Drawable)
			throw new IllegalArgumentException("Drawable replacements are deprecated since msposed 2.1. Use DrawableLoader instead.");
			...
			if (incrementMethodDepth("getDrawable") == 1) {
				...
				if (replacement instanceof DrawableLoader) {
					try {
						...
						if (result != null)
							return result;
					} catch (Throwable t) { msposedBridge.log(t); }
				} else if (replacement instanceof Integer) {
			...
			if (callbacks != null) {
				...
				if (value.type == TypedValue.TYPE_STRING) {
					...
					if (components.length == 3)
						...
						msposedBridge.log("Unexpected resource path \"" + value.string.toString()
								+ "\" for resource id 0x" + Integer.toHexString(id));
				} else {
					msposedBridge.log(new NotFoundException("Could not find file name for resource id 0x") + Integer.toHexString(id));
				}
	...
	public void getValue(int id, TypedValue outValue, boolean resolveRefs) throws NotFoundException {
		...
		if (replacement instanceof XResForwarder) {
			...
			if (replacement != null) {
				msposedBridge.log("Replacement of resource ID #0x" + Integer.toHexString(id) + " escaped because of deprecated replacement. Please use XResForwarder instead.");
			}
	...
	private static int translateResId(int id, XResources origRes, Resources repRes) {
		try {
			...
			if (!repResDefined && origResId == 0 && !entryType.equals("id")) {
				msposedBridge.log(entryType + "/" + entryName + " is neither defined in module nor in original resources");
				return 0;
			}
	...
	private static int translateAttrId(String attrName, XResources origRes) {
		...
		try {
			...
			msposedBridge.log("Attribute " + attrName + " not found in original resources");
		}
		return origAttrId;
	}
