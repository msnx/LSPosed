...
public class Startup {
    ...
    public static void bootstrapmsposed() {
        // Initialize the msposed framework
        try {
            startBootstrapHook(msposedInit.startsSystemServer);
            msposedInit.loadLegacyModules();
        } catch (Throwable t) {
            Utils.logE("error during msposed initialization", t);
        }
    }

    public static void initmsposed(boolean isSystem, String processName, String appDir, ILSPApplicationService service) {
        // init logger
        ApplicationServiceClient.Init(service, processName);
        msposedBridge.initXResources();
        msposedInit.startsSystemServer = isSystem;
