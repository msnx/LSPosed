        ...
        for (Object[] caller : callers) {
            try {
                ...
                if ("<init>".equals(caller[1])) {
                    method = msposedHelpers.findConstructorExactIfExists((String) caller[0], cl, params);
                } else {
                    method = msposedHelpers.findMethodExactIfExists((String) caller[0], cl, (String) caller[1], params);
                }
                if (method != null) {
