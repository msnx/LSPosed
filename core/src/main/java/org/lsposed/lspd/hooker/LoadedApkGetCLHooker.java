...
public class LoadedApkGetCLHooker implements XposedInterface.Hooker {
    ...
    public static void afterHookedMethod(XposedInterface.AfterHookCallback callback) {
        ...
        try {
            ...
            if (!isFirstPackage && !msposedHelpers.getBooleanField(loadedApk, "mIncludeCode")) {
                Hookers.logD("LoadedApk#<init> mIncludeCode == false: " + mAppDir);
                return;
            }

            if (!isFirstPackage && !msposedInit.getLoadedModules().getOrDefault(packageName, Optional.of("")).isPresent()) {
                return;
            }

            XC_LoadPackage.LoadPackageParam lpparam = new XC_LoadPackage.LoadPackageParam(
                    msposedBridge.sLoadedPackageCallbacks);
            ...
            if (isFirstPackage && msposedInit.getLoadedModules().getOrDefault(packageName, Optional.empty()).isPresent()) {
                hookNewXSP(lpparam);
            }
    ...
    private static void hookNewXSP(XC_LoadPackage.LoadPackageParam lpparam) {
        ...
        if (xposedminversion > 92 || xposedsharedprefs) {
            ...
            msposedHelpers.findAndHookMethod("android.app.ContextImpl", lpparam.classLoader, "checkMode", int.class, new XC_MethodHook() {
                @Override
            ...
            msposedHelpers.findAndHookMethod("android.app.ContextImpl", lpparam.classLoader, "getPreferencesDir", new XC_MethodReplacement() {
                @Override
